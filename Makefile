...
$(BONEOS_ISO): $(BONEOS_BOOT_BIN)
	...
	$(GRUB_MKRESCUE) --output=BoneOS.iso boot
...
qemu_compile: $(BONEOS_BIN)
	$(QEMU) -kernel $(BONEOS_BIN) -display sdl -k en-us -s -m $(MEMORY_USE_RAM)

qemu_iso: $(BONEOS_BIN) $(BONEOS_ISO)
	$(QEMU) -cdrom $(BONEOS_ISO) -k en-us -s -m $(MEMORY_USE_RAM)
...
configure-help:
	...
	@echo '--ld=LD             use LD linker [$(LD)]'
	@echo '--ar=AR             use AR archiver [$(AR)]'
	@echo '--as=AS             use AS assembler [$(AS)]'
	@echo '--grub-mkconfig=MC  use grub-mkconfig compiler [$(GRUB_MKCONFIG)]'
	@echo ''
	...
	@echo ''
	@echo '--m=MEMORY_USE_RAM  compile kernel that has'
	@echo '                    MEMORY_USE_RAM memory.'
	@echo ''
...
configure-set:
	...
	@echo "MEMORY_USE_RAM := $(MEMORY_USE_RAM)" >> $(GENERATED_CONFIG)
